...
export default function AddExpense() {
  ...
  const [group, setGroup] = useState(groupIdFromUrl || '');
  ...
  // Fetch members when group ID from URL is available
  useEffect(() => {
    if (groupIdFromUrl && isOrgListLoaded) {
      setGroup(groupIdFromUrl);
      fetchMembers(groupIdFromUrl);
    }
  }, [groupIdFromUrl, isOrgListLoaded]);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setAmount('');
        setDescription('');
        setGroup(groupIdFromUrl || ''); // Keep the group if it came from URL
        setSplitPercentage('');
        setSplitWith([]);
      } else {
        ...
        <div>
          ...
          {organizations.length > 0 ? (
            <Select 
              ...
              value={group} 
              required
              disabled={!!groupIdFromUrl} // Disable if group ID is provided in URL
            >
