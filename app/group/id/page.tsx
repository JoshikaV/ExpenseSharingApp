...
function GroupPage() {
  const { id } = useParams();
  const { organizations, userMemberships, isLoaded: orgLoaded } = useOrganizationList({
    userMemberships: {
      infinite: true,
    },
  });
  ...
  }, [id, user, userLoaded, orgLoaded, userMemberships?.data, router, toast]);
  ...
  const handleDeleteGroup = async () => {
    ...
    if (data.success) {
      ...
      toast({ title: data.error || 'Failed to delete group.' });
    }
  };
    ...
    <div className="p-6 max-w-4xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        ...
        {isAdmin && (
          ...
          >
            <Trash2 className="inline-block w-4 h-4 mr-1" /> Delete Group
          </Button>
        )}
      </div>
